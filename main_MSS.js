...
//   terrainProvider: await CesiumTerrainProvider.fromIonAssetId(1), // 世界地形
 ...
 animation: true,   // 右下角播放控制
 timeline: true,    // 下方時間軸可拖拉
 shouldAnimate: true,
  baseLayerPicker: false
});
viewer.scene.globe.enableLighting = true;  // 光影
...
const bingAerialProvider = new BingMapsImageryProvider({
  url: "https://dev.virtualearth.net",
  mapStyle: BingMapsStyle.AERIAL
});
// 加一個預設底圖 layer（Carto）
let basemapLayer = viewer.imageryLayers.addImageryProvider(cartoProvider);

// Terrain
// const worldTerrain = createWorldTerrain();
// const ellipsoidTerrain = new EllipsoidTerrainProvider();
// viewer.terrainProvider = worldTerrain; // 預設開啟 terrain
...
// ===============================
// === loading FET moving data ===
// ===============================
// GeoJsonDataSource.load(
//   "data/moving_geojson_forDT/moving_all_20250923_1450.geojson",
//   { clampToGround: true }
// ).then(function (ds) {
...
//     if (e.polygon) {
//       e.polygon.material = Color.RED.withAlpha(0.6);
//       e.polygon.outline = false;
//     }
//   });
...
const start = JulianDate.fromIso8601("2025-09-23T14:50:00");
const stop  = JulianDate.fromIso8601("2025-09-23T16:30:00");

viewer.clock.startTime = start.clone();
viewer.clock.stopTime = stop.clone();
viewer.clock.currentTime = start.clone();
viewer.clock.multiplier = 300;     // 1 秒 = 5 分鐘
...
function julianToUTC8Date(julianDate) {
  const utc = JulianDate.toDate(julianDate); // 這是 UTC 的 Date
  return new Date(utc.getTime() + TZ_OFFSET_MS);    // 轉成 UTC+8 的「顯示用」Date
}

// 以 UTC getter 取值，避免被瀏覽器本地時區二次轉換
...
const avm = viewer.animation.viewModel;

avm.dateFormatter = (julianDate) => {
  const d = julianToUTC8Date(julianDate);
  return `${formatDateUTC(d)} (UTC+8)`;
};

avm.timeFormatter = (julianDate) => {
  const d = julianToUTC8Date(julianDate);
...
fetch("data/moving_geojson_forDT/moving_manifest.json")
  .then(r => r.json())
  .then(data => {
    manifest = data;
...
function onTick(clock) {
  ...
  if (currentDataSource?.name === best.file) return;
  ...
  // 載入新的
  GeoJsonDataSource.load(
    "data/moving_geojson_forDT/" + best.file,
    { clampToGround: true }
  ).then(ds => {
    ds.name = best.file;
    styleMovingLayer(ds);
...
function styleMovingLayer(ds) {
  ds.entities.values.forEach(e => {
    if (!e.polygon) return;

    const v = e.properties.people_move_all.getValue();

    let color;
    if (v === 0) color = Color.TRANSPARENT;
    else if (v < 5) color = Color.GREEN.withAlpha(0.5);
    else if (v < 20) color = Color.YELLOW.withAlpha(0.6);
    else color = Color.RED.withAlpha(0.7);
